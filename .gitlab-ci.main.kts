 ...
 */
val siteUrl = "https://kotlin-resources.opensavvy.dev/"
...
gitlabCi {
	...
	// endregion
	// region Example: Kotlin Multiplatform

	val kmpBuild by job(stage = build) {
		jsBrowser()
		...
		afterScript {
			shell("mkdir -p website")
			shell("mv examples/kotlin-multiplatform/app/build/dist/js/productionExecutable/* website")
			shell("./gitlab/ci/review-url.sh website/index.html >kmp.env")
		}
		...
		environment {
			name("review/${Value.Commit.Ref.slug}/example-kmp")
			url($$"$url")
			tier(Development)
		}
